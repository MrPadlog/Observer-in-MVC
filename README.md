...
class Subscriber: # observers
    ''' Initialisations '''
    def __init__(self, name):
        self.name = name # gets the name, i.e. comboboxentry or chkbox_chkd etc.
    ''' Updates '''
    def update(self, entry): # actual update method
        print("The actual {0} is {1}.".format(self.name, entry))
...
class Publisher: # observed
    ''' Initialisations '''
    def __init__(self):
        self.subscribers = set()
    ''' Registrations '''
    def register(self, who):
        self.subscribers.add(who) # who want's to observe
    ''' Initialisations '''
    def unregister(self, who):
        self.subscribers.discard(who) # who doesn't want to observe anymore
    def dispatch(self, entry):
        for subscriber in self.subscribers:
            subscriber.update(entry) # call update for each subscriber
...
# needs to be changed, so it reads the changed entries from the controller
# a method to write the actual state in the model needs to be implemented
class Drivers:
    pub = Publisher()

    combobox_entry = Subscriber('Comboboxentry')
    
    pub.register(combobox_entry)
    
    pub.dispatch("It's lunchtime!") # here, the actual entry shall be dispatched
